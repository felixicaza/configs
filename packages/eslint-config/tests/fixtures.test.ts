...
beforeAll(async(): Promise<void> => {
  fixturesTempDir = await mkdtemp(join(tmpdir(), 'eslint-config-fixtures-'))
})
...
runWithConfig(
  ...
  [
    ...
    },
    {
      files: ['**/*.yaml', '**/*.yml'],
      rules: {
        'yml/indent': ['error', 2, { indicatorValueIndent: 2 }]
      }
    },
    {
      files: ['**/*.json'],
      rules: {
        'jsonc/indent': ['error', 3]
      }
    }
  ]
)
  ...
  it.concurrent(
    ...
    async({ expect }): Promise<void> => {
      ...
      const configContent = [
        ...
        '',
        `export default felixicaza(${JSON.stringify(options, null, 2)}, ${JSON.stringify(userConfigs, null, 2)})`,
        ''
      ].join('\n')

      await writeFile(join(target, 'eslint.config.ts'), configContent, 'utf-8')

      const result = await x('eslint', ['.', '--fix'], {
        nodeOptions: {
          cwd: target,
          env: {
            ...process.env,
      ...
      if (result.exitCode !== 0) {
        throw new Error(`eslint failed (${result.exitCode})\nSTDOUT:\n${result.stdout}\nSTDERR:\n${result.stderr}`)
      }
